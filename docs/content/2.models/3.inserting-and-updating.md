...
## Inserting records and creating new model objects

New model objects can be created by using the `new` operator to create an object of your model's class. You can then use `save()` to persist it to your database.

```ts
const pet = new Pet({ name: "Zuko", type: "bird" });
await pet.save();
```

You can also do this in a single command by using `create()` to create a new model instance object and immediately save it do the database.
